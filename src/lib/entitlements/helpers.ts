 ...
 *
 * Entitlements are queried at the ORGANIZATION level, not user level.
...
/** Arbor product ID for entitlements */
const PRODUCT_ID = "arbor";
...
async function fetchOrganizationEntitlements(
  ...
  for (const ent of result.data.entitlements) {
    // Check product-specific tier first (arbor:tier), then shared tier
    if (ent.featureKey === `${PRODUCT_ID}:tier` || ent.featureKey === "tier") {
      ...
      // Handle both product-specific (arbor:max_repositories) and shared (max_repositories)
 ...
 *
 * SECURITY: Throws EntitlementsUnavailableError if Aether is down.
 * @knipignore - exported for direct limit checks
 */
