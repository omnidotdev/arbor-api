 ...
 *
 * - Create: Any authenticated user can create a repo (with tier limits for org repos)
          ...
          if (scope === "create") {
            ...
            if (!billingBypassSlugs.includes(organization.slug)) {
              if (organization.tier === "free") {
                if (
                  organization.repositories.length >= FREE_TIER_MAX_REPOSITORIES
                )
...
const RepositoryPlugin = wrapPlans({
  Mutation: {
    createRepository: validatePermissions("repository", "create"),
